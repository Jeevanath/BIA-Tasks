...
def hill_climbing(lmin, lmax, fn):
	trace = []
	for _ in range(20):
		neighbours = [normal((lmin, lmax) , 5) for number in range (100)] 
		for n in range(0, 100):
			for m in range(0, 2):
				# print("before ",neighbours[n][m])
				neighbours[n][m]= clamp(neighbours[n][m], lmin, lmax)
				# print("after ",neighbours[n][m])
...
for fn in functions_list:
	...
	data = hill_climbing(fn.xmin, fn.xmax, fn)
	print(data)
